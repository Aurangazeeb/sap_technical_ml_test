...
so ``find_similar_products(product_id, num_similar, state)`` runs
...
def test_public_signature_matches_readme_spec():
    """find_similar_products(product_id, num_similar, state)."""
    sig = inspect.signature(find_similar_products)
    assert list(sig.parameters.keys()) == ["product_id", "num_similar", "state"]


# ── Sanity ────────────────────────────────────────────────────────────────────

def test_known_product_not_in_results(state):
    """Query product must not appear in its own results."""
    results = find_similar_products("prod_000", 3, state)
...
def test_num_similar_respected(state, k):
    """Return exactly num_similar product IDs."""
    assert len(find_similar_products("prod_001", k, state)) == k


# ── Invalid product_id raises ValueError ─────────────────────────────────────

def test_invalid_product_id_raises_value_error(state):
    """Unknown product_id must raise ValueError."""
    with pytest.raises(ValueError, match="prod_nonexistent"):
        find_similar_products("prod_nonexistent", 3, state)


# ── Determinism ───────────────────────────────────────────────────────────────

def test_results_are_deterministic(state):
    """Same inputs must always produce the same output."""
    assert find_similar_products("prod_002", 3, state) == find_similar_products("prod_002", 3, state)


# ── Return type ───────────────────────────────────────────────────────────────

def test_returns_list_of_strings(state):
    """Return value must be a list[str]."""
    results = find_similar_products("prod_003", 2, state)
...
def test_no_duplicate_ids_in_results(state):
    """All returned product IDs must be unique."""
    results = find_similar_products("prod_004", 5, state)
